...
const texto =   ${item.nombre || ""}   ${item.descripcion || ""}   ${item.categoria || ""}   ${item.direccion || ""}  .toLowerCase();
...
c.innerHTML +=   <div class="tarjeta">   ${img ?<img src="${img}">: ""}   <h3>${it.nombre}</h3>   <p>${it.descripcion || ""}</p>   ${   it.imagenes?.length > 1   ?<button onclick='abrirGaleria(${JSON.stringify(it.imagenes)})'>${TEXTOS[LANG].verMas}</button>  : ""   }   </div>  ;
});
}
